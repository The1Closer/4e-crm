...
export async function PATCH(req: NextRequest, context: RouteContext) {
  ...
  try {
    const slug = await generateUniqueClaimCategorySlug(name, categoryId)
    ...
    if (error || !data) {
      throw new Error(error?.message || 'Could not update the category.')
    }
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : 'Could not update the category.',
      },
      { status: 400 }
    )
  }
