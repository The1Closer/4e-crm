...
export default function ContractsEditorCore() {
  ...
  function resetEditorForNewDocument(nextUrl: string, nextName: string) {
  ...
  function handleLocalFileChange(file: File | null) {
    ...
    resetEditorForNewDocument(objectUrl, file.name.replace(/\.pdf$/i, ''))
  }
  ...
  function onLoadError(error: Error) {
    ...
    setMessage('Failed to load this PDF. Make sure the source file is valid and reachable.')
  }
  ...
  function addCheck(page: number, x = 60, y = 120) {
    const item: Annotation = {
      id: uid(),
      page,
      type: 'check',
  ...
  function autoDetectFields(pageNumber: number) {
    ...
    requestAnimationFrame(() => {
      ...
      for (const span of spans) {
        ...
        if (CHECK_PATTERNS.some((p) => p.test(text))) {
          newItems.push({
            id: uid(),
            page: pageNumber,
            type: 'check',
  ...
  async function saveAnnotatedPdf() {
    if (!loadedPdfUrl) {
      setMessage('No source document is loaded.')
      return
    }
    ...
    try {
      ...
      if (jobId) {
        ...
        const safeBase = slugifyFileName(documentName || 'signed-document')
        ...
        const { error } = await supabase
          ...
          .insert({
            ...
            file_url: fileUrl,
            file_path: filePath,
            document_type: documentId || localFileName ? 'Edited Document' : 'Signed Document',
            is_signed: true,
        ...
        setMessage('Document saved back to this job.')
      } else {
    ...
    <main className="min-h-screen bg-gray-50 p-6 md:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-gray-600">
                Open a template, open an existing job document, or upload a one-time PDF from your device.
              </p>
            </div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-[1fr_320px_auto]">
            <input
            ...
            <label className="flex cursor-pointer items-center justify-center rounded-xl border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-900 transition hover:bg-gray-100">
              <input
                type="file"
                accept="application/pdf"
                onChange={(e) => handleLocalFileChange(e.target.files?.[0] ?? null)}
                className="hidden"
              />
              Choose PDF File
            </label>
          ...
          {!hasSource ? (
            <div className="mt-4 rounded-xl border border-yellow-200 bg-yellow-50 p-4 text-sm text-yellow-800">
              No document source was provided. Open this editor from Templates, from a job document, or choose a one-time PDF from your device.
            </div>
          ) : null}
            ...
            <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              {!selectedAnnotation ? (
                <p className="mt-3 text-sm text-gray-600">
                  Click a field to edit, move, or resize it.
                </p>
              ) : (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateAnnotation(selectedAnnotation.id, {
                            width: Number(e.target.value || selectedAnnotation.width),
                          })
                        }
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateAnnotation(selectedAnnotation.id, {
                            height: Number(e.target.value || selectedAnnotation.height),
                          })
                        }
                      />
                    </div>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            {!hasSource ? (
              <div className="rounded-xl border border-dashed border-gray-300 p-10 text-center text-sm text-gray-600">
                Open this editor from a template, an existing homeowner/job document, or choose a PDF file from your device.
              </div>
            ) : documentLoadError ? (
              <div className="rounded-xl border border-dashed border-red-300 p-10 text-center text-sm text-red-700">
                          ...
                          >
                            ...
                            {pageAnnotations.map((annotation) => (
                              <Rnd
                                ...
                                minWidth={annotation.type === 'check' ? 20 : 40}
                                minHeight={annotation.type === 'check' ? 20 : 24}
                                bounds="parent"
                              ...
                              >
                                {annotation.type === 'signature' && annotation.imageDataUrl ? (
                                  ...
                                  <div className="flex h-full w-full items-center justify-center text-center text-xs font-semibold text-blue-700 pointer-events-none">
                                    {annotation.value ?? 'Signature'}
                                  </div>
                                ) : annotation.type === 'check' ? (
                                  <div
                                    className="flex h-full w-full items-center justify-center font-bold text-green-700 pointer-events-none"
                                    style={{
                                  ...
                                  <div
                                    className="h-full w-full overflow-hidden whitespace-pre-wrap break-words text-black pointer-events-none"
                                    style={{
